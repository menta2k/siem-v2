...
- [x] T189 Release workflow in `.github/workflows/release.yml` (modeled on v1's) — separate from CI by design (the merge gate must not publish); GHCR images for apiserver/logproc/retentiond (one `Dockerfile.backend` with SERVICE arg), frontend (new Nuxt SSR image, runtime-configured API base) and wirefilter-svc (amd64-only until QEMU is worth it); multi-arch Go images, gha build cache, branch/SHA tags on main + semver on `v*` tags. Tag-only packages job: version-stamped static binaries (`internal/version`; stamp verified by grep — `-X` on a missing symbol is silently ignored), nfpm deb+rpm with systemd units and `config|noreplace` configs, layout asserted via dpkg-deb; release job publishes with sha256 checksums. Dry-run locally: stamp verified in binary AND container log, deb/rpm built and inspected, frontend image serves 200
- [x] T190 First-start admin bootstrap hardening in production use — the container deployment surfaced the full failure chain and each link was fixed or documented: stale links from scrollback (every apiserver restart re-issues while unredeemed → always take the LAST BOOTSTRAP log line), a preview fetch that failed on a dead API hostname rendering as "link not usable", and the shared-dev-database case where bootstrap correctly no-ops because seeded accounts can sign in
- [x] T191 Interactive `setup.sh` — asks names only (users, database, admin email, public URLs), generates every password/key with the right shape per consumer (DSN-safe alnum for PG/S3, base64url JWT/ingest, EXACTLY-32-byte MFA key with asserted length), writes `deploy/compose/.env` mode 600, refuses to overwrite without explicit yes; found+fixed a SIGPIPE-under-pipefail bug in its own generator during testing
- [x] T192 Production deployment on siem.infra.verax.net (`/srv/siem-v2`, via SSH) — full app+tunnel profiles running; diagnosed and fixed live: cloudflared origin must use CONTAINER ports (`frontend:3000`, not host 3002), `.env` browser-facing URLs must match the actual public hostname (were pointing at deleted `api.server-lab.eu` records), and the decisive one — cloudflared evaluates ingress rules IN ORDER, so the no-path frontend rule first swallowed `/api` and `/ingest` (proof: the API returned the Nuxt HTML shell with 200); reordered in the Zero Trust dashboard → bootstrap invite redeemed, admin signed in over production TLS
- [x] T193 `deploy/compose/docker-compose.prod.yml` — production override running the PUBLISHED GHCR images (`build: !reset`, `pull_policy: always`, `IMAGE_TAG` pins a release, latest tracks main); prod host needs no Go/node/cargo and runs exactly what CI built; cut over live on prod and verified in the browser (all five services on ghcr.io images, sources dialog present at siem.server-lab.eu)
- [x] T194 Sources registration dialog in `frontend/app/pages/sources.vue` — the page was read-only although `POST /api/v1/sources` (manage_sources) existed since onboarding; visible the moment a tenant without seeded sources opened it. Dialog carries the onboarding gate's fields with their REASONS as hints ("without cadence, silence is undetectable"); verified locally and on prod
- [x] T198 The "nginx stopped" incident (2026-08-20): nothing had stopped — BIG-IP logged local time parsed as UTC, placing every F5 event ~3h in the future where newest-first sorts pinned F5-only flows above everything. Fixed with v1's BIGIP_UTC_OFFSET ported into the collector (round-trip tested, +0300 on prod); verified post-fix: 3,959 F5 events stamped in the last 3 real minutes
- [x] T199 Four silent-failure paths found via the incident and its Logpush follow-up, all fixed and deployed: (1) EvaluateSilence was never CALLED in production — logproc now runs a minute-cadence silence loop logging went-silent/recovered transitions; (2) RegisterSource wiped the silence clock on every accepted delivery — now idempotent; (3) refused ingest deliveries (401) left no trace — now logged rate-limited per feed (a Logpush job once pushed for an hour against a suspected-bad credential with zero server-side evidence); (4) parse failures only bumped an unrendered rate — now one ERROR per provider per minute carrying the parser's own reason
- [x] T200 Health chain completed to the UI: logproc syncs observed source health (healthy/silent/awaiting + last_record_at) to Postgres every minute (SourceRepo.RecordDelivery had zero callers — the Sources page could never leave "Awaiting first record"); feed creation auto-registers its matching log_source row; existing prod feeds backfilled; silence judged at ACCEPT time not consumer time (a backlog drain made every source look silent while deliveries poured in)
- [x] T201 Dashboard aggregates a bounded 24h window ending NOW instead of "newest 1000 flows" — future-stamped records could monopolize every panel (Records per provider showed only f5asm while nginx flowed normally)
- [x] T202 Cloudflare Logpush live on prod via the v2 feed — destination updated with the header_Authorization construction, validated, and ramped to ~250-record batches at several/second; DataDome came free (Worker-subrequest verdicts on CF records): within the first hour 565k cloudflare flows, 76k cloudflare+datadome ParentRayID-bridged joins, alongside 629k f5asm+nginx exact joins; dashboard verified showing all four providers with real ASN attribution (VIVACOM, A1BG, TELENOR) on production traffic
- [x] T203 The road from "0 CF correlation" to the first four-layer production flows — three stacked bottlenecks, each exposed by fixing the previous: (1) prod's late_arrival_window was the 60s dev value while Logpush delivers minutes apart → raised to 5m; (2) raw originals stored one HTTP insert per record capped the pipeline at ~370 rec/s (backlog draining ~10 msgs/min, a 12h catch-up) → one insert per batch, ~140× faster drain; (3) the catch-up burst closed 200k windows in one flush tick and stored them one insert at a time UNDER the pipeline mutex — the consumer takes the same lock, so acks froze and the pipeline deadlocked against its own recovery → storage moved outside the lock + BulkStore (500 flows/insert). Result on live jobs.bg traffic in 15 minutes: 54,404 cloudflare+datadome, 338 cloudflare+f5asm, 108 cloudflare+f5asm+nginx, and 56 COMPLETE cloudflare+datadome+f5asm+nginx four-layer exact-tier flows
- [x] T204 Straggler merge (the Amender, wired) — the user's "same ray already stored → correlate them" was the design's own dormant intent: flows closing for a key stored within a 1h horizon load-merge-restore as Amended (FR-018), search dedups to the newest version per flow id, and Get pins the newest version (an unsorted limit-1 returned an ARBITRARY one — merging into a stale copy drops events). Covers Cloudflare's own 3-7 minute log latency when it exceeds the window
- [x] T205 Per-user timezone + hour-format preference (v1's design ported) — app-bar clock menu (curated zones + browser default, locale/24h/12h, ticking preview), localStorage-persisted, display-only by design; all timestamp call sites render through usePrefs; unit-tested and live-verified (Sofia→UTC shifted the search table -3h instantly)
- [x] T206 Per-tenant ingest filters (v1's design ported) — closed field/operator sets over request_host/request_path, max 64 rules; matched records dropped BEFORE any storage (no raw, no event, no flow — the one deliberate Constitution II exception, so every surrounding failure fails OPEN); 30s-refresh cache in logproc; GET/POST /api/v1/filters behind manage_sources validated by the exact ingest-path compiler with the full rule set audited; /filters page with sentence-rendered rules, overly-broad red flag and v1's "cannot be recovered" copy; verified end-to-end (filtered record: 200 to sender, logged as dropped, absent from events AND raw; kept record fully ingested)
- [x] T207 Search time-range filter — presets (15m/1h/24h/15d/30d) resolving at search time plus a custom datetime-local range (empty To = now); UI-only fields stripped from the API payload, backend from/to window unchanged; verified live (preset narrowed 47→1 fresh flow; custom window isolated exactly yesterday's 15:00-15:30 fixtures)
- [x] T208 CI: arm64 images built only for version tags — branch pushes are amd64-only (the deployment target); tags stay multi-arch. Cut a main-push Release build from ~8-12min to 1m53s
- [x] T209 Correlation window survives restart (FR-023) — Snapshot/Restore existed and were scenario-tested but logproc NEVER CALLED THEM: the window lived only in memory, so every deploy silently dropped flows still mid-correlation (found when a manually-triggered CF SQLi block, ray a2df4b3d4da6d0e8, was captured to raw but a deploy restart 3min later wiped its open flow before it could be stored). logproc now restores the window from Valkey on boot (clearing the key so a crash can't replay stale state) and persists on graceful shutdown after the final flush; Pipeline.Snapshot drains not-yet-correlated events in too. Verified live on prod: 2,533 states persisted→restored across a restart, then a re-triggered SQLi block (ray a2df657f0f55d0f2) flowed edge→raw→window→searchable blocked flow. Known gap: a HARD crash still loses the in-flight window, but raw records survive for future reprocessing
- [x] T210 Per-user timezone + hour-format preference (v1 ported) — app-bar clock menu (curated zones + browser default, locale/24h/12h, ticking preview), localStorage, display-only; all timestamp call sites via usePrefs; unit-tested; live-verified (Sofia→UTC shifted the table -3h instantly)
- [x] T211 Per-tenant ingest filters (v1 ported) — closed field/op sets (request_host/request_path; equals/prefix/suffix/contains), max 64; matched records dropped BEFORE any storage (no raw/event/flow — one deliberate Constitution II exception, everything around it fails OPEN); 30s-refresh cache in logproc; GET/POST /api/v1/filters behind manage_sources validated by the ingest compiler, full rule set audited; /filters page with sentence-rendered rules, overly-broad flag, "cannot be recovered" copy; verified end-to-end (filtered: 200+logged+absent; kept: ingested)
- [x] T212 Search time-range filter — presets (15m/1h/24h/15d/30d) resolving at search time + custom datetime-local range (empty To = now); UI-only fields stripped from the payload; verified live (preset 47→1; custom isolated yesterday 15:00-15:30)
- [x] T213 First-start bootstrap durability across restart (FR-023) — window Snapshot/Restore existed but logproc never called them, so every deploy dropped in-flight flows (a manually-triggered CF SQLi block, ray a2df4b3d, was captured to raw but lost when a restart wiped its open window). logproc now restores the window from Valkey on boot (clears the key) and persists on graceful shutdown; verified live (2,533 states persisted→restored; a re-triggered block flowed edge→raw→window→searchable)
- [x] T214 F5 host extraction — ASM's KV log has no host field; the vhost lives only in the captured request blob (Host header). Parser now scrapes it (preferring a dedicated field), the CRLF-as-literal-escape form handled; verified live (fresh f5+nginx flows now show host)
- [x] T215 Dashboard "Records per provider" from a direct event count, not a 1000-flow sample — the sample showed cloudflare:12 while the true 1h counts are cloudflare 1.33M (highest) > f5asm > datadome > nginx; now `record_kind:=event | stats by (provider)` over 24h; verified live (cloudflare 5.06M, dominant)
- [x] T216 Completeness relative to the terminating layer — "complete" required all 4 layers, so an edge-BLOCK (origin never saw it) was mislabeled partial and the Complete filter returned ~nothing. Coverage now judged only up to the terminating layer: edge block expects just edge (complete), an allowed flow missing origin is partial, a pass-through expects all 4
- [x] T217 Correlation throughput + self-deadlock fixes found under the CF Logpush firehose — (a) raw originals stored one HTTP insert per record capped the pipeline at ~370 rec/s (12h backlog); one insert per batch, ~140x faster drain. (b) Flush closed 200k windows/tick and stored them one-at-a-time UNDER the pipeline mutex, freezing the consumer; storage moved outside the lock + BulkStore (500 flows/insert)
- [x] T218 **THE correlation invariant fix** — 315k f5asm+nginx flows/hour orphaned from Cloudflare, violating CF→DD→F5→nginx. Root cause (traced on prod ray a2dfaa0e7ba969dd): Correlate ran union-find over ONE 5s batch then cleared it, but CF Logpush arrives minutes after the real-time origin, so the bridge row (shares origin ray, canonically keyed on the smaller PARENT ray) filed with the edge half while the origin half stayed keyed apart. Fix: incremental identifier→window-key index attaches records across ANY time gap and merges windows when a bridge spans them; index persists to Valkey with the window/merge state. Also widened late_arrival_window 5m→10m. Verified live: orphans 315k→**5**, joined 460→**46,424**, four-layer complete 2→**45,341** — the full CF+DataDome+F5+nginx flow is now the NORMAL case
- [x] T219 CI: arm64 images built only on version tags — branch pushes amd64-only (the deploy target); cut a main-push build from ~8-12min to 1m53s
- [x] T220 Production deployment infrastructure proven end-to-end — GHCR published images via docker-compose.prod.yml, cloudflared tunnel with path-routed ingress, per-feed nginx + F5-collector-container delivery on live jobs.bg traffic; the running system now reconstructs four-provider flows at scale exactly as specified
- [x] T221 Dashboard selectable time range (15m/1h/24h, default 1h) with true aggregation — every panel (outcomes, completeness, terminating layer, correlation quality, provider counts, top sources, top networks) is now a direct stats query over EVERY flow in the window, not the newest-1000 sample (under a minute at production volume). Verified live: 15m 188k / 1h 1.39M / 24h 7.83M flows, panels re-scope instantly
- [x] T222 Completeness boundary = the deepest layer the request is known to have reached, not a fixed 4 (which mislabeled edge-blocks partial and returned ~nothing for Complete). Boundary = deepest layer with a record (origin logged it => passed everything above, incl. a response-side WAF block), OR the terminating layer for request-path blocks, OR the origin for an allowed non-cached request. Subsumes: (a) edge/DataDome blocks complete at their layer; (b) cache-hit edge flows complete (CacheCacheStatus carried through the CF parser — a hit never contacts origin); (c) the F5 "Illegal redirection attempt" case the user found — F5 blocks the ORIGIN'S 302 RESPONSE, so nginx legitimately has a record and the flow is a complete 4-layer, not a confusing 3-layer-terminated-at-F5. Live result: F5-participating flows 107,195 complete/4-layer per 15m vs a tiny partial tail; DataDome-blocks confirmed complete (902/902), never counted as partial
- [x] T195 nginx production onboarding via Vector (jobs host, 4 vhosts) — per-feed URL + token in the sink; two real bugs found live: Vector's disk-buffer floor is 268435488 bytes (exactly 256 MiB is rejected with status=78/CONFIG, taking the whole pipeline down — dead-letter buffer raised to 512 MiB with the floor documented), and the tunnel's path rules must ALL sit above the no-path catch-all (a half-reorder left /ingest routed to the frontend; Vector's disk buffer held everything and drained the moment routing was fixed — zero loss). Live jobs.bg traffic searchable in the SIEM
- [x] T196 F5 syslog collector container in `deploy/compose/docker-compose.yml` (`vector-f5`, profile `f5`, ported from v1's BIG-IP collector) — syslog→authenticated HTTP against the f5asm feed over the COMPOSE network (never out through the edge and back); `f5asm.toml` takes feed URL+token from env with NO defaults so the container refuses to start rather than delivering to nowhere (validated both ways against the pinned image); syslog port bound 127.0.0.1 unless configured, firewall warning carried from v1; disk buffer on a named volume
- [x] T225 CF `managedChallenge` (and connectionClose/forceConnectionClose) mapped, not dumped to "unknown" (user-found on the Outcomes card). Cloudflare's current ruleset writes SecurityAction in camelCase; the action table only had snake_case, so lowercasing produced an unmatched concatenated token and ~9k managed challenges/hour — the single most common security action on this traffic — fell through to the `unknown` outcome instead of `challenged`. Added the concatenated forms alongside the existing ones; unit-tested the camelCase family. Live traffic then surfaced the rest of the managedChallenge* family: *NonInteractiveSolved/*InteractiveSolved (visitor passed → challenge_passed) and *Bypassed (an exception let it through → allowed) — all mapped, taking residual CF `unknown` to ~zero. DataDome's `unknown` left as-is by design: those are subrequest_status 499 (client closed the connection before DataDome answered) — genuinely no verdict, not a mapping gap

- [x] T224 Search discoverability + early-close for terminal flows (user-driven). (a) A flow spans several rays (visitor + each Worker subrequest) but only the canonical ray was indexed searchable, so pasting the SUBREQUEST ray F5/nginx show found nothing though correlation was correct; now a multi-value `ray_ids` field indexes every ray, the ray filter word-matches it, and a `query` quick-find ORs ray_ids/vendor_request_ids/flow_id (one box for any id from any console). Verified live: subrequest-ray and F5-support-id both resolve to the flow. Caveat: `ray_ids` only exists on flows written post-deploy (VL can't backfill a field). (b) Early-close: the window closed a flow only on all-4-layers OR the 10m timeout, so a terminal block (edge/DataDome) — which no deeper layer will ever amend — waited the full window before becoming searchable. Window now also closes a flow the moment it is "terminally complete": a terminating record present AND every layer up to that stop reported (shallowest terminating layer = where the request died; layer orders are contiguous from edge=0). Edge blocks close on the next 5s tick; DataDome blocks close once their upstream edge record is in; allowed/partial flows are untouched (still all-layers-or-timeout). Unit-tested (edge block, DataDome-after-edge, missing-upstream-waits, allowed-not-closed)

- [x] T223 Raw provider evidence as a first-class, separately-retained artifact (user request) — every request's UNMODIFIED provider records are now stored in a DEDICATED VictoriaLogs instance (`victorialogs-raw`, :9430) with its own retention class, distinct from flows/events; VL has no per-record TTL, so a shorter window needs its own instance. (a) StoreRaw/RawForFlow route to the raw client (falls back to the main instance on single-node dev stacks); the raw_id linkage bug fixed so records store under the event's provider id (cf:<ray>, f5:<support_id>) instead of provider:hash, which is what RawForFlow keys on. (b) `GET /api/v1/flows/{flowID}/raw` (view_raw, audited) returns each contributing record's original payload; the flow drawer gained a "Raw evidence" panel that lazily fetches and shows the originals per provider (masked-field note when redaction applied). (c) Retention is UI-CONFIGURABLE: `raw_retention_days` on the tenant (default 7, bounds 1–365), GET/POST `/api/v1/settings/raw-retention` behind manage_retention, a /settings admin page; retentiond enforces it hourly (and at boot) via VL's `/delete/run_task` delete-by-time against the raw instance, per-tenant and best-effort. `RAW_RETENTION_MAX_DAYS` (default 30) caps the instance flag as a ceiling. Backend build/vet/tests green; frontend typecheck/eslint clean

- [x] T197 Two collector bugs found live on prod and fixed — (1) the vector image's default command loads its own bundled DEMO pipeline, so the mounted config was never read and the collector looked alive while shipping joke syslog to the console; explicit `command: --config /etc/vector/vector.toml` now. (2) Vector's syslog codec DROPS non-UTF-8 frames, and ASM's captured request bodies are binary precisely on the interesting events (exploit payloads) — observed live as "Failed framing bytes" from BIG-IP; switched to a raw socket source (v1's choice, reason now recorded), parser proven against the real capture to scrape KV past the syslog header. After fix: 0 framing errors, ~50-60 ASM records/sec flowing
